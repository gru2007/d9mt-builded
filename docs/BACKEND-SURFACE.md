...
| `blitImageView(dstView, VkOffset3D[2], srcView, VkOffset3D[2], VkFilter)` | scaling/format-converting StretchRect; 2D_ARRAY views with packedSwizzle; can mirror (offsets). NB the front-end passes views created with TRANSFER_SRC/DST usage, so the sampled source view is built from the image, not taken off `srcView` | Our fullscreen-triangle sample pass (blit_vs/ps), honoring swizzle + filter; a multisampled source is resolved into a cached 1-sample image first |
